...
def pop_data(country_df):
    ...
    for i in range(len(country_age) - 1):
        description_field.append('Women population ' + str(country_age[i]) + ' to ' + str(country_age[i+1]) + ' years old.')
        description_field.append('Men population ' + str(country_age[i]) + ' to ' + str(country_age[i+1]) + ' years old.')
